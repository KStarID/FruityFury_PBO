package fruityfury.controller;

import fruityfury.model.Banana;
import fruityfury.model.Bomb;
import fruityfury.model.Player;
import fruityfury.model.Bomb1;
import fruityfury.model.Field;
import fruityfury.model.Apple;
import fruityfury.model.Strawberry;
import fruityfury.model.Melon;
import fruityfury.model.Orange;
...
import fruityfury.view.ApplicationView;
import fruityfury.view.EndGameView;
	...
	private void loadAudio() {
		try {
			...
			bgSound.open(AudioSystem.getAudioInputStream(
					new BufferedInputStream(getClass().getResourceAsStream("/fruityfury/resources/game_music.wav"))));
		} catch (Exception e) {
			e.printStackTrace();
		}
		try {
			...
			slashSound.open(AudioSystem.getAudioInputStream(
					new BufferedInputStream(getClass().getResourceAsStream("/fruityfury/resources/slash.wav"))));
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
			...
			public void mouseDragged(MouseEvent e) {
				...
				if (field.getObject().insideBounds(e.getX(), e.getY())) {
					...
					field.getSlash().setEndPosition(e.getX(), e.getY());
                                                                                            appView.repaint(); // Repaint the component to 
                                                                }
			}
